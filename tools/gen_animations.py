...
def generate_spritesheet(name, prompt, frames=4, frame_size=64, output_path=None):
    ...
    """
    # 构造水平序列提示词
    full_prompt = f"{prompt}. Show {frames} animation frames in a horizontal sequence from left to right, each frame {frame_size}x{frame_size} pixels, no gaps, clean white background, pixel art style, consistent character across all frames"
