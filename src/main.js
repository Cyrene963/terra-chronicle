...
/* 简单动画精灵:横向帧拼接,自动播放/停止,方向翻转 */
class AnimatedSprite {
  constructor(sheetTex, idleTex, frameCount, fps, anchorY) {
    this.container = new PIXI.Container();
    this.sprite = new PIXI.Sprite();
    this.sprite.anchor.set(0.5, anchorY ?? 1.0);
    this.container.addChild(this.sprite);

    this.sheetTex = sheetTex;          // 走路循环贴图(横向4帧)
    this.idleTex = idleTex;            // 静止贴图
    this.frameCount = frameCount || 4;
    this.fps = fps || 8;
    this.frameIndex = 0;
    this.frameTime = 0;
    this.playing = false;
    ...
    // 初始静止帧
    if(this.idleTex) this.sprite.texture = this.idleTex;
  }

  play() {
    if(this.playing || !this.sheetTex) return;
    this.playing = true;
    this.frameIndex = 0;
    this.frameTime = 0;
    this._updateFrame();
  ...
  _updateFrame() {
    if(!this.sheetTex) return;
    const frameW = this.sheetTex.width / this.frameCount;
    const frameH = this.sheetTex.height;
    const rect = new PIXI.Rectangle(this.frameIndex * frameW, 0, frameW, frameH);
    this.sprite.texture = new PIXI.Texture({
      source: this.sheetTex.source,
      frame: rect
    });
  }
}
...
const ASSETS = {
  player:   { src: 'assets/sprites/player_idle.png', walkSheet: 'assets/sprites/player_walk_sheet.png',
              w: 46,  h: 73,  anchorY: 1.0, faceDir: -1, frames: 4 },
  ...
  beast_water:{ src:'assets/sprites/beast_water.png', walkSheet: 'assets/sprites/beast_water_walk_sheet.png',
                w: 62,  h: 64,  anchorY: 0.86, frames: 4 },
  beast_fire:{ src:'assets/sprites/beast_fire.png', walkSheet: 'assets/sprites/beast_fire_walk_sheet.png',
               w: 58,  h: 66,  anchorY: 0.86, frames: 4 },
  ...
  tiles: {
    ...
    // Diagonal water-land transitions
    water_diag_tl: { src: 'assets/sprites/water_diag_tl.png' },  // top-left grass, bottom-right water
    water_diag_tr: { src: 'assets/sprites/water_diag_tr.png' },  // top-right grass, bottom-left water
    water_diag_bl: { src: 'assets/sprites/water_diag_bl.png' },  // bottom-left grass, top-right water
    water_diag_br: { src: 'assets/sprites/water_diag_br.png' },  // bottom-right grass, top-left water
  },
};
...
const grid=[], blocked=new Set(), tileMeta={}, diagonalTiles={};
...
function genMap(){
  ...
  // 检测对角线水陆过渡
  detectDiagonalTransitions();
}
genMap();

/* 检测对角线水陆过渡瓦片 */
function detectDiagonalTransitions(){
  const isWater=(x,y)=>grid[y]&&grid[y][x]==='w';
  const isLand=(x,y)=>grid[y]&&grid[y][x]&&grid[y][x]!=='w';

  for(let y=0;y<MAP;y++){
    for(let x=0;x<MAP;x++){
      // 检查 LAND 瓦片,看是否需要对角线切角
      ...
      // 对角线检测:检测"内凹水角" - 对角方向是水,但两个正交邻居不全是水
      // 这种情况下需要对角线过渡瓦片来平滑视觉

      // 西北角有水凸入(NW是水,但N或W至少一个是陆地) → 用 water_diag_tl(左上角水色)
      if(nw && !n && !w){
        diagonalTiles[x+','+y] = 'water_diag_tl';
      }
      // 东北角有水凸入(NE是水,但N或E至少一个是陆地) → 用 water_diag_tr(右上角水色)
      else if(ne && !n && !e){
        diagonalTiles[x+','+y] = 'water_diag_tr';
      }
      // 西南角有水凸入(SW是水,但S或W至少一个是陆地) → 用 water_diag_bl(左下角水色)
      else if(sw && !s && !w){
        diagonalTiles[x+','+y] = 'water_diag_bl';
      }
      // 东南角有水凸入(SE是水,但S或E至少一个是陆地) → 用 water_diag_br(右下角水色)
      else if(se && !s && !e){
        diagonalTiles[x+','+y] = 'water_diag_br';
      }
    }
  }
}
...
/* ================= 4. PIXI 启动 ================= */
// FIX: Return to v9-v11 simple initialization (resizeTo: window handles everything)
(async ()=>{
...
/* resizeTo: window handles all resize automatically */
...
for(let y=0;y<MAP;y++)for(let x=0;x<MAP;x++){
  ...
  // 检查是否有对角线过渡瓦片
  let tileName = null;
  if(diagonalTiles[key]){
    tileName = diagonalTiles[key];
  } else {
    const tileMap = {g:'grass', G:'grass', s:'soil', w:'water', b:'sand', p:'plot'};
    tileName = tileMap[k];
  }

  const t=ASSETS.tiles[tileName];
  ...
  sp._ph=r*6.28;                               // 水面相位
  sp._isDiag=!!diagonalTiles[key];            // 标记对角线瓦片(不参与水流动画)
...
function makeNode(kind){
  ...
  // 尝试为灵兽加载动画精灵
  if((kind==='beast_water' || kind==='beast_fire') && a.walkSheet && a.frames){
    Promise.all([
      loadTex(a.src).catch(()=>null),
      loadTex(a.walkSheet).catch(()=>null)
    ]).then(([idleTex, walkTex]) => {
      if(walkTex) {
        const anim = new AnimatedSprite(walkTex, idleTex, a.frames, 8, a.anchorY);
        anim.sprite.width = a.w;
        anim.sprite.height = a.h;
        node.addChild(anim.container);
        node._anim = anim;
        node._body = anim.sprite;  // 引用给其他代码用
      } else if(idleTex) {
        const sp=new PIXI.Sprite(); sp.anchor.set(.5, a.anchorY??1);
        sp.texture=idleTex; sp.width=a.w; sp.height=a.h;
        node.addChild(sp); node._body=sp;
      }
    });
    node._graded=true; node._kind=kind;
    return node;
  }
...
function animateWalk(dt,moving){
  // 更新 AnimatedSprite 帧动画
  if(player._anim) {
    player._anim.update(dt);
    if(moving && !player._anim.playing) player._anim.play();
    else if(!moving && player._anim.playing) player._anim.stop();
  }
...
function commandTo(wx,wy){
  // Try to trigger random encounter on grass tiles (5% chance)
...
function beastStep(dt){
  ...
  // 更新 AnimatedSprite 帧动画
  if(beast._anim) {
    beast._anim.update(dt);
    if(moving && !beast._anim.playing) beast._anim.play();
    else if(!moving && beast._anim.playing) beast._anim.stop();
  }
  ...
  if(beastAI.state==='idle'){
    ...
    if(beastAI.t<=0){
      ...
      if(pc){
        pc.watered=true;
        const isHot=greenhouse.hotCrops.has(beastAI.target);
        if(isHot){
          // STEAM SYNERGY: Hot crop + water = double yield + steam effect
          pc.boost=true; pc.steamBoosted=true;
          spawnSteam(pc.node.x, pc.node.y-16, true);
          // Add glow to crop
          if(pc.node._body) pc.node._body.tint=0xfff8e0;
        } else {
          pc.boost=true;
          toastHint('水灵兽灌溉了一块田 · 生长加速');
        }
      }
...
function spawnSplash(key){                          // 在耕地处喷涌一束蓝色水花(上抛+重力下落)
  ...
  if(window.TerraSound) TerraSound.play('splash', 0.6);
  for(let i=0;i<n;i++){
...
/* ================= 8.7 灵兽繁育 + 火灵兽(工坊熔炉增益) + 生态链反应 ================= */
let fireBeast=null, fireAI=null, forgeHot=false, waterEvolved=false;
const embers=[], heatHazes=[], steamParticles=[];

// Fire beast greenhouse effect tracking
const greenhouse={active:false, hotCrops:new Set()};
...
/* —— Heat Haze Particles (Fire Beast Greenhouse Effect) —— */
function spawnHeatHaze(x,y){
  const h=new PIXI.Graphics();
  h.circle(0,0,3+Math.random()*2).fill({color:0xff9a4a,alpha:.25});
  h.x=x+(Math.random()*32-16); h.y=y-8+(Math.random()*12-6);
  h._vy=-12-Math.random()*8; h._vx=(Math.random()*8-4); h._life=0; h._wavePhase=Math.random()*6.28;
  overlayL.addChild(h); heatHazes.push(h);
}
function stepHeatHazes(dt){
  for(let i=heatHazes.length-1;i>=0;i--){
    const h=heatHazes[i]; h._life+=dt; h._wavePhase+=dt*3;
    h.x+=h._vx*dt + Math.sin(h._wavePhase)*15*dt;  // Wavy rising motion
    h.y+=h._vy*dt;
    h.alpha=Math.max(0,.25-h._life*.4);
    if(h._life>.7){overlayL.removeChild(h);heatHazes.splice(i,1);}
  }
}

/* —— Steam Particles (Water Beast + Hot Crop Synergy) —— */
function spawnSteam(x,y,intense){
  const n=intense ? 8+(Math.random()*4|0) : 4+(Math.random()*3|0);
  for(let i=0;i<n;i++){
    const s=new PIXI.Graphics();
    s.circle(0,0,3+Math.random()*3).fill({color:0xf8fafc,alpha:.7});
    s.x=x+(Math.random()*24-12); s.y=y-4;
    s._vy=-40-Math.random()*(intense?60:30); s._vx=(Math.random()*20-10); s._life=0;
    overlayL.addChild(s); steamParticles.push(s);
  }
  if(intense) toastHint('💧 蒸汽滋养！产量翻倍');
}
function stepSteam(dt){
  for(let i=steamParticles.length-1;i>=0;i--){
    const s=steamParticles[i]; s._life+=dt; s._vy+=20*dt;  // Slight upward deceleration
    s.x+=s._vx*dt; s.y+=s._vy*dt;
    s.alpha=Math.max(0,.7-s._life*.9);
    if(s._life>.8){overlayL.removeChild(s);steamParticles.splice(i,1);}
  }
}

/* —— Fire Beast Greenhouse Detection —— */
function updateGreenhouse(){
  if(!fireBeast) return;
  const fp=furnacePos();
  const nearFurnace=Math.hypot(fireBeast.x-fp.x,fireBeast.y-fp.y)<3*TS;
  greenhouse.active=nearFurnace;
  greenhouse.hotCrops.clear();

  if(nearFurnace){
    // Find all crops within 5×5 tile radius
    const fbx=Math.floor(fireBeast.x/TS), fby=Math.floor(fireBeast.y/TS);
    for(const key in planted){
      const [cx,cy]=key.split(',').map(Number);
      if(Math.abs(cx-fbx)<=5 && Math.abs(cy-fby)<=5){
        greenhouse.hotCrops.add(key);
      }
    }
    // Show hint when effect first activates
    if(greenhouse.hotCrops.size>0 && !greenhouse._hintShown){
      toastHint('🔥 温室效应激活！作物加速生长');
      greenhouse._hintShown=true;
    }
  } else {
    greenhouse._hintShown=false;
  }
}

/* —— Spawn Heat Haze Particles Around Hot Crops —— */
let hazeSpawnClock=0;
function stepGreenhouseVisuals(dt){
  if(!greenhouse.active || greenhouse.hotCrops.size===0) return;
  hazeSpawnClock+=dt;
  if(hazeSpawnClock>=0.12){  // Spawn haze every 120ms
    hazeSpawnClock=0;
    for(const key of greenhouse.hotCrops){
      const pc=planted[key];
      if(pc && !pc.mature && Math.random()<0.3){  // 30% chance per crop per tick
        spawnHeatHaze(pc.node.x, pc.node.y-20);
      }
    }
  }
}
...
function fireStep(dt){
  ...
  // 更新 AnimatedSprite 帧动画
  if(fireBeast._anim) {
    fireBeast._anim.update(dt);
    if(moving && !fireBeast._anim.playing) fireBeast._anim.play();
    else if(!moving && fireBeast._anim.playing) fireBeast._anim.stop();
  }
...
app.ticker.add(tk=>{
  ...
  updateCamera(dt);
  /* 作物生长累计(浇水 boost 加速 + greenhouse 2×速度&2×肥力消耗) + 灵兽 AI + 连续伐木 */
  updateGreenhouse();
  for(const key in planted){ const pc=planted[key];
    if(!pc.mature){
      const isHot=greenhouse.hotCrops.has(key);
      const growSpeed = (pc.boost?1.8:1) * (isHot?2:1);  // Greenhouse doubles growth speed
      pc.grown=(pc.grown||0)+dt*timeScale*growSpeed;
      if(pc.grown>=GROW_SECONDS) pc.mature=true;
    }
  }
  if(entered){
    beastStep(dt); stepWaterDrops(dt); stepTrails(dt);
    fireStep(dt); stepEmbers(dt);
    stepHeatHazes(dt); stepSteam(dt); stepGreenhouseVisuals(dt);
  ...
  // Tutorial system checks (runs every frame when needed)
  if(entered && window.Tutorial) {
    ...
    const hasMoved = Math.abs(player.x - 23*TS) > 10 || Math.abs(player.y - 26.6*TS) > 10;

    // Check if near harvestable crop
    const key = playerTileKey();
    const nearCrop = planted[key] && planted[key].mature;

    // Check if can craft
    const canCraft = wheat >= 3 && wood >= 1;
...
if (window.CaptureIntegration) {
  ...
  console.info('[Terra] Capture system integrated');
}
...
function chop(o){
  ...
  if(o.hp>0){ toastHint('咔 —— 再砍一下'); if(window.TerraSound) TerraSound.play('chop', 0.8); return; }
  ...
  if(window.TerraSound) TerraSound.play('chop');
...
function interactFarm(key){
  ...
  if(!pc){                                        // 播种
    ...
    if(window.TerraSound) TerraSound.play('plant');
  } else if(pc.mature){                           // 收获:质量继承产地肥力
    const meta=tileMeta[key];
    const quantity = pc.steamBoosted ? 2 : 1;  // STEAM SYNERGY: Double yield
    ...
    if(pc.steamBoosted){
      toastHint(`💧🔥 蒸汽滋养！收获 星麦×${quantity} · 产地肥力 ${meta.fert}`);
    } else {
      toastHint(`收获 星麦 · 产地肥力 ${meta.fert}`);
    }
    if(window.TerraSound) TerraSound.play('harvest');
...
$('cardReveal').onclick=()=>{
  $('cardReveal').classList.remove('on');
  if(window.TerraSound) TerraSound.play('whoosh');
};
updateDock();
...
function enterWorld(){
  ...
  if(window.TerraSound) {
    setTimeout(() => TerraSound.startMusic(), 1200);
  }
  ...
  if(window.Tutorial) {
    Tutorial.checkTriggers({ hasEntered: true });
  }
}
$('enter').onclick=enterWorld;

// Sound controls setup - Simple mute button only
function setupSoundControls() {
  const muteBtn = $('muteBtn');

  if(!muteBtn || !window.TerraSound) return;

  // Mute button - toggle sound on/off
  muteBtn.addEventListener('click', () => {
    const muted = TerraSound.toggleMute();
    muteBtn.textContent = muted ? '🔇' : '🔊';
    if(!muted && window.TerraSound) TerraSound.play('click');
  });
