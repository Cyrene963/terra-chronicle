...
(async () => {
  console.log('🎮 Terra Chronicle Feature Test Suite (Puppeteer)\n'));
  ...
  try {
    ...
    await new Promise(r => setTimeout(r, 3000); // Wait for assets
    ...
    console.log(`  Mute toggle works: ${soundInGameTest.mute_toggle_works ? '✅' : '❌'} (${soundInGameTest.initial_icon} → ${soundInGameTest.muted_icon})`));
    ...
    const tutorialCheck = await page.evaluate(() => {
      return {
        ...
        hint_count: window.tutorial && window.tutorial.hints ? window.tutorial.hints.length : 0
      };
    }));
    ...
    console.log(`  Has hints defined: ${tutorialCheck.tutorial_has_hints ? '✅' : '❌'} (${tutorialCheck.hint_count} hints)`));
    ...
    const recipeCheck = await page.evaluate(() => {
      ...
      const samples = RECIPES.slice(0, 3).map(r => `${r.name}(${r.wheat}w/${r.wood}wd)`));
    ...
    console.log(`  Samples: ${recipeCheck.sample_recipes}`));
    ...
    sections.forEach(section => {
      ...
      Object.entries(section.data).forEach(([key, value]) => {
        if (typeof value === 'boolean') {
          totalTests++;
          if (value === true) passedTests++;
          const icon = value ? '✅' : '❌';
          console.log(`  ${icon} ${key.replace(/_/g, ' ')}`));
        } else if (typeof value === 'number' && key.includes('count')) {
          totalTests++;
